...
from backbone.dale_dendrites.modules.sparse_dale_dendritic_layers import \
    SparseDaleAbsoluteMaxGatingDendriticLayer
...
class SparseDaleDendriticMLP(nn.Module):
    """
    ...
    def __init__(
        ...
        kw, kw_percent_on=[0.05, 0.05], context_percent_on=1.0, dendrite_weight_sparsity=0.95,
        ...
        output_inhib_units=1, i_iid_i=False, apply_to_dendrites=False, apply_heterogeneous_dropout=False,
    ):
        ...
        self.apply_heterogeneous_dropout = apply_heterogeneous_dropout
        ...
        if use_shunting:
            ...
            dendritic_layer_class = SparseDaleAbsoluteMaxGatingDendriticLayer
        ...
        for i in range(len(self.hidden_sizes)):
            ...
            self._activation_counts[f'layer_{i}_classwise'] = torch.zeros(self.output_size, hidden_sizes[i][0])
            ...
            if weight_init == "modified":
                ...
                if i == 0:
                    ...
                    self._init_sparse_weights(
                        curr_dend,
                        1 - kw_percent_on[i] if kw else 0.0
                    )
    ...
    def forward(self, x, context=None, disable_dropout=False, y=None, classwise_dropout=False, return_activations=False):
        ...
        for i, (layer, activation) in enumerate(zip(self._layers, self._activations)):
            ...
            # Apply Dropout
            if self.apply_heterogeneous_dropout and not disable_dropout:
                if self.training and self._keep_probs:
                    if classwise_dropout and y is not None:
                        layer_mask = torch.rand(orig_act.shape[1]) < self._keep_probs[f'layer_{i}_classwise'][y]
            ...
            if self.training:
                ...
                if y is not None:
                    for class_idx in range(self.output_size):
                        sel_idx = y == class_idx
                        self._activation_counts[f'layer_{i}_classwise'][class_idx] += (x[sel_idx] > 0).sum(dim=0).cpu()

        if self._single_output_head:
            if return_activations:
                return self._output_layers[0](x), activations
    ...
    def init_weights(self, **args):
        for layer in self._layers:
            layer.module.init_weights(**args)
    ...
    def get_si_params(self, apply_to_dendrites=False) -> torch.Tensor:
        """
        Returns all the parameters concatenated in a single tensor.
        :return: parameters tensor (??)
        """
        ...
        for name, pp in self.named_parameters():
            if apply_to_dendrites or "segment" not in name:
                params.append(pp.view(-1))
        return torch.cat(params)
    ...
    def get_si_grads(self, apply_to_dendrites=False) -> torch.Tensor:
        """
        Returns all the gradients concatenated in a single tensor.
        :return: gradients tensor (??)
        """
    ...
    def rezero_weights(self):
        for layer in self._layers:
            layer.rezero_weights()


# model = SparseDaleDendriticMLP(
#     input_size=784,
#     output_size=10,
...
#
# # model._layers[0].rezero_weights()
#
# Wex = model._layers[1].module.Wex
#
# print((Wex == 0).sum())
# print(torch.numel(Wex))
