...
from backbone.dendrites.utils.sparse_weights import SparseWeights, rezero_weights
...
class DendriticMLP(nn.Module):
    ...
    def __init__(
        ...
        kw, kw_percent_on=[0.05, 0.05], context_percent_on=1.0,
        ...
        for i in range(len(self.hidden_sizes)):
            self._activation_counts[f'layer_{i}'] = torch.zeros(hidden_sizes[i])
        ...
        for out_size in output_size:
            ...
            if weight_init == "modified":
                self._init_sparse_weights(
                    output_linear, 1 - kw_percent_on[-1] if kw else 0.0)
    ...
    def forward(self, x, context=None, disable_dropout=False, return_activations=False):
        assert (context is not None) or (self.num_segments == 0)

        activations = {}

        for n, (layer, activation) in enumerate(zip(self._layers, self._activations)):
            ...
            if self.apply_heterogeneous_dropout and not disable_dropout:
                if self.training and self._keep_probs:
                    # print('Applying Heterogeneous Dropout')
                    # layer_mask = torch.Tensor([Binomial(probs=prob).sample() for prob in self._keep_probs[f'layer_{i}']])
                    layer_mask = torch.rand(orig_act.shape[1]) < self._keep_probs[f'layer_{n}']
                    layer_mask = layer_mask.to(x.device)
                    orig_act *= layer_mask

            x = activation(orig_act)
            activations[f'layer_{n}'] = x

            # Update the activation count
            if self.training:
                self._activation_counts[f'layer_{n}'] += (x > 0).sum(dim=0).cpu()
