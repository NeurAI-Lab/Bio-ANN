...
def evaluate(model: ContinualModel, dataset: ContinualDataset, last=False, eval_ema=False, ema_model=None) -> Tuple[list, list]:
    ...
    for k, test_loader in enumerate(dataset.test_loaders):
        ...
        for data in test_loader:
            ...
            if 'bio' in model.NAME:
                inputs = inputs.view(inputs.shape[0], -1)

                if hasattr(model, 'use_semantic_groups'):
                    context = F.one_hot(super_class, num_classes=20).type(torch.float)
                else:
                    context = model.infer_context_fn(inputs)
                    if isinstance(context, tuple):
                        context = context[1]
                        # context = context[2]
                context = context.to(inputs.device)
                outputs = curr_model(inputs, context)
            else:
...
def train(model: ContinualModel, dataset: ContinualDataset,
    ...
    if model.NAME != 'icarl' and model.NAME != 'pnn' and not 'bio' in model.NAME and dataset.NAME != 'gcil-cifar100':
    ...
    if args.save_model:
        ...
        if hasattr(model, 'contexts'):
            torch.save(model.contexts, os.path.join(model_dir, f'contexts.ph'))